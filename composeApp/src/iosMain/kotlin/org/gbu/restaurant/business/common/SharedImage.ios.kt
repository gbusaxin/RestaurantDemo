...
actual class SharedImage(private val image: UIImage?) {
    ...
    actual fun toByteArray(): ByteArray? {
        ...
        return if (compressedImage != null) {
            val imageData = UIImageJPEGRepresentation(compressedImage, 0.99)
    ...
    actual fun toImageBitmap(): ImageBitmap? {
        ...
        return if (byteArray != null) Image.makeFromEncoded(byteArray).toComposeImageBitmap()
        else null
    }
}
...
private fun compressImage(uiImage: UIImage): UIImage? {
    ...
    while (imageSize > 500000) {
        ...
        tryCounter -= 1
        if (tryCounter == 0) return null
    }

    if (imageSize < 500000) return photo
    else return null
}
